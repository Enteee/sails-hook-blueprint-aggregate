...
var actionUtil = {
  ...
  */
   ...
   */
  parseModelAttrFromParam: function (req, name) {

    const model = actionUtil.parseModel(req);
    const param = req.param(name) || req.options[name];
    var parsed;

    // check if keys are model attributes
    function isValid(attr){
console.log('checking:', attr);
      return !!(model._attributes[attr])
    }
  
    const obj = tryToParseJSON(param);
    if(_.isEmpty(parsed) && isArray(obj)){
      parsed = _(obj)
        .filter(isValid)
        .value();
    }
    if(_.isEmpty(parsed) && isObject(obj)){
      parsed = {};
      _(obj)
        .keys()
        .filter(isValid)
    ...
    if(_.isEmpty(parsed) && isString(param)){
      if(isValid(param)) parsed = [param];
    }

    return (!_.isEmpty(parsed)) ? parsed : undefined;
  },
